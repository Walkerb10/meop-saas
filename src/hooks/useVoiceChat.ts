...
export function useVoiceChat({ voiceId, onTranscript, onError }: UseVoiceChatOptions) {
  ...
  // Keep the recording format consistent (important for Safari/mp4)
  ...
  const getAIResponse = useCallback(async (userText: string): Promise<string> => {
    ...
    if (!response.ok) {
      ...
      throw new Error((errorData as any).error || 'AI request failed');
    }
  ...
  const speakText = useCallback(async (text: string): Promise<void> => {
    ...
    if (!response.ok) {
      ...
      throw new Error((error as any).error || 'TTS request failed');
    }
  ...
  const transcribeAudio = useCallback(async (audioBlob: Blob): Promise<string> => {
    ...
    const ext = recordingFileExtRef.current || 'webm';
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error((errorData as any).error || 'Transcription failed');
    }
  ...
  const startListening = useCallback(async () => {
    try {
      ...
      recordingFileExtRef.current = mimeType.includes('mp4') ? 'mp4' : 'webm';
