      ...
      <div className="mb-4 p-4 rounded-xl bg-muted/30">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            {todayTask.description && <p className="text-sm text-muted-foreground mt-1">{todayTask.description}</p>}
            {todayTask.estimated_minutes && <Badge variant="outline" className="mt-2 text-xs gap-1">
                <Clock className="h-3 w-3" />
                {formatDuration(todayTask.estimated_minutes)}
              </Badge>}
          </div>
          {todayTask.status === 'completed' && <Check className="h-5 w-5 text-green-500 shrink-0" />}
        </div>

        {/* Bonus task (pulled forward after completing One Thing) */}
        {isTodayOriginalCompleted && bonusTask && <div className="mt-4 pt-4 border-t border-border/50">
            <div className="flex items-start gap-3">
        ...
        {isTodayOriginalCompleted && !bonusTask && !nextTask && isCurrentUser && <div className="mt-4 pt-4 border-t border-border/50">
