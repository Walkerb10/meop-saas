...
import { Clock, ArrowDown, GitBranch, Play, Zap, ChevronRight, ArrowLeft, Trash2, Loader2, Plus, MessageSquare, Pencil, X } from 'lucide-react';
...
const ScheduledActions = () => {
  ...
  const { automations, loading, executeAutomation, deleteAutomation, createAutomation } = useAutomations();
  ...
  const buildTriggerLabel = () => {
    if (formData.frequency === 'weekly') {
      return `Every ${formData.dayOfWeek} at ${formData.time}`;
    } else if (formData.frequency === 'monthly') {
      return `Monthly on day ${formData.dayOfMonth} at ${formData.time}`;
    }
    return `Daily at ${formData.time}`;
  };
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-semibold">
              {showCreateForm ? 'Create Automation' : selectedAutomation ? selectedAutomation.name : 'Automations'}
            </h1>
            {!selectedAutomation && !showCreateForm && (
          ...
          {showCreateForm ? (
            ...
            >
              ...
              <Button 
                ...
                disabled={creating || !formData.name.trim() || !formData.message.trim()}
              >
                {creating ? (
